...
let removeBtnNode = () => {
    let col = document.createElement('div');
        ...
        col.innerHTML = `<button type="button" class="btn remove_row_variation btn-icon btn-rounded btn-sm btn-light-danger">
...
let optionsNode = (key = null) => {
    ...
    let input_options = document.createElement('input');
        ...
        input_options.setAttribute('readonly','true');
        if(key != null){
            input_options.setAttribute('value',key);
        }
        input_options.classList.add('form-control','form-control-sm','text-black');
...
let variationsNode = () => {
    ...
    let input_variations = document.createElement('input');
        input_variations.classList.add('form-control','form-control-sm','tag_variation');
    ...
    for (let x in permutations) {
        ...
        new_variantRow.innerHTML = `
                        <div class="col-3">
                            ...
                            <input type="text" class="form-control border-0 form-control-sm text-black" value="${
                        ...
                        <div class="col-8">
                            <div class="mb-2">
                                ...
                                <input type="number" name="variations[${x}][qty]" required min="1" class="form-control form-control-sm">
                            </div>
                        </div>
                        <div class="col-1">
                        <button type="button" class="btn remove_variation_box btn-icon btn-rounded btn-sm btn-light-danger">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="6" height="6"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round"
                                            class="feather feather-trash-2">
...
function appendNewVaritation(key = {} , current_values = [],combainted = []){
    ...
    let row = document.createElement('div');
        ...
        row.selectedItem = key['item'];
        row.setAttribute('data-value',key['text'] );

        let variation = variationsNode();
        let tagify = variation.tag;
        let options   = optionsNode(key['text']);
        ...
        tagify.on('add', function (e){
            ...
            appendInputVariation($('#variations_combination')[0],combantioned_variations);
