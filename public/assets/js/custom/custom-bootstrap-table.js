...
const YOUTUBE_LINK_TYPE = 2;
const VIDEO_UPLOAD_TYPE = 3;
const OTHER_LINK_TYPE = 4;
        ...
        if (row.file.length > 0) {
            $.each(row.file, function (key, data) {
                ...
                let html = $(".edit_clone_file_type_div")
                    .clone()
                    .show()
                    .removeClass("edit_clone_file_type_div");

                $("#editModal .edit-extra-files").append(html);
                ...
                switch (Number(data.type)) {
                    ...
                    case OTHER_LINK_TYPE:
                        ...
                        html.find(".file_link").val(data.file_url);
                        break;
                    case VIDEO_CORNER_TYPE:
                        html.find(".file_type")
                            .val("video_corner_link")
                            .trigger("change");
                        html.find(".file_name").val(data.file_name);
                        html.find(".video_corner_url").val(data.file_url);
                        html.find(".download_link").val(data.download_link);
                        break;
    ...
    "click .edit-data": function (e, value, row, index) {
        ...
        $("#edit_class_id").on("select2:unselecting", function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = `${baseUrl}/delete-exam-class/${exam_id}/${class_id}`;
...
window.assignmentEvents = {
    "click .edit-data": function (e, value, row, index) {
        ...
        if (row.resubmission) {
            $("#edit_resubmission_allowed")
                .prop("checked", true)
                .trigger("change");
            $("#edit_extra_days_for_resubmission").val(
                row.extra_days_for_resubmission
            );
        } else {
            $("#edit_resubmission_allowed")
                .prop("checked", false)
        ...
        if (row.file) {
            $.each(row.file, function (key, data) {
                html_file += `
                <div class="file">
                    <a target="_blank" href="${data.file_url}" class="m-1">${data.file_name}</a> 
                    <span class="fa fa-times text-danger remove-assignment-file" data-id="${data.id}"></span>
                    <br><br>
                </div>`;
            });
...
window.announcementEvents = {
    "click .editdata": function (e, value, row, index) {
        ...
        if (row.assign == "Subject") {
            $("#edit_set_data")
                .val("class_section")
                .trigger("change", [row.get_data]);
            $("#edit_class_section_id")
                .val(row.assign_to["class_section_id"])
                .trigger("change", [row.assign_to["subject_id"]]);
        ...
        if (row.file) {
            $.each(row.file, function (key, data) {
                html_file +=
                    '<div class="file"><a target="_blank" href="' +
                    data.file_url +
                    '" class="m-1">' +
                    data.file_name +
                    '</a> <span class="fa fa-times text-danger remove-assignment-file" data-id=' +
                    data.id +
                    "></span><br><br></div>";
            });
...
window.classSubjectEvents = {
    "click .edit-data": function (e, value, row, index) {
        ...
        $(".core-subject-id")
            .find('option[data-medium-id="' + row.medium_id + '"]')
            .show();
        ...
        $(".edit-core-subject-id")
            .find('option[data-medium-id="' + row.medium_id + '"]')
            .show();

        $(".edit-elective-subject-name").find("option").hide();
        $(".edit-elective-subject-name")
            .find('option[data-medium-id="' + row.medium_id + '"]')
            .show();
        ...
        $.each(row.core_subjects, function (key, value) {
            ...
            core_subject
                .find("select")
                .find(`option[value='${value.subject_id}']`)
        ...
        if (row.elective_subject_groups.length > 0) {
            ...
            $.each(row.elective_subject_groups, function (key, group) {
                ...
                subjectGroup
                    .find(".edit-total-selectable-subject")
                    .val(group.total_selectable_subjects);
                subjectGroup
                    .find(".remove-elective-subject-group")
                    .attr("data-id", group.id);

                //Second loop will be used for Subjects inside Groups
                subjectGroup
                    .find(".edit-elective-subject-group-id")
                    .val(group.id);
                $.each(
                    group.elective_subjects,
                    function (key, elective_subject) {
                        if (key === 0) {
                            subjectGroup
                                .find(".edit-elective-subject-name:first")
                                .val(elective_subject.subject.id);
                            subjectGroup
                                .find(".edit-elective-subject-name:first")
                                .siblings(".edit-elective-subject-class-id")
                                .val(elective_subject.id);
                        } else if (key === 1) {
                            subjectGroup
                                .find(".edit-elective-subject-name:eq(1)")
                                .val(elective_subject.subject.id);
                            subjectGroup
                                .find(".edit-elective-subject-name:eq(1)")
                                .siblings(".edit-elective-subject-class-id")
                            ...
                            let electiveSubject = cloneNewElectiveSubject(
                                electiveSubjectButton
                            );
                            electiveSubject.insertBefore(
                                subjectGroup.find(".add-new-elective-subject")
                            );
                            electiveSubject
                                .find(".edit-elective-subject-name")
                                .val(elective_subject.subject.id);
                            electiveSubject
                                .find(".edit-elective-subject-name")
                                .siblings(".edit-elective-subject-class-id")
                            ...
                            electiveSubject
                                .find(".edit-elective-subject-name")
                                .siblings(".remove-elective-subject")
                                .attr("data-id", elective_subject.id);
                        }
                    }
                );
            });
...
window.parentEvents = {
    "click .editdata": function (e, value, row, index) {
        ...
        $("input[name=gender][value=" + row.gender + "].edit").prop(
            "checked",
            true
        );
        $("#email").val(row.email);
    ...
    "click .editdata": function (e, value, row, index) {
        ...
        setTimeout(function () {
            ...
            $.each(row.dynamic_data_field, function (key, value) {
                ...
                var id_with_hash =
                    "#" + Object.keys(row.dynamic_data_field[key])[0];
                $("#file-" + id + "").val("");
                $("#" + id + "-div").css("display", "none");
        ...
        setTimeout(function () {
            $("#edit_father_first_name")
                .val(row.father_first_name)
                .attr("readonly", true);
            $("#edit_father_last_name")
                .val(row.father_last_name)
                .attr("readonly", true);
            $("#edit_father_mobile")
                .val(row.father_mobile)
                .attr("readonly", true);
            $("#edit_father_dob").val(row.father_dob).attr("readonly", true);
            $("#edit_father_occupation")
                .val(row.father_occupation)
                .attr("readonly", true);
        ...
        setTimeout(function () {
            $("#edit_mother_first_name")
                .val(row.mother_first_name)
                .attr("readonly", true);
            $("#edit_mother_last_name")
                .val(row.mother_last_name)
                .attr("readonly", true);
            $("#edit_mother_mobile")
                .val(row.mother_mobile)
                .attr("readonly", true);
            $("#edit_mother_dob").val(row.mother_dob).attr("readonly", true);
            $("#edit_mother_occupation")
                .val(row.mother_occupation)
                .attr("readonly", true);
        ...
        //Adding delay to fill data so that select2 code and this code don't conflict each other
        setTimeout(function () {
            $("#edit_guardian_first_name")
                .val(row.guardian_first_name)
                .attr("readonly", true);
            $("#edit_guardian_last_name")
                .val(row.guardian_last_name)
                .attr("readonly", true);
            $("#edit_guardian_mobile")
                .val(row.guardian_mobile)
                .attr("readonly", true);
            ...
            $("#edit_guardian_dob")
                .val(row.guardian_dob)
                .attr("readonly", true);
            $("#edit_guardian_occupation")
                .val(row.guardian_occupation)
                .attr("readonly", true);
...
window.examMarksEvents = {
    "click .edit-data": function (e, value, row, index) {
        ...
        $.each(row.data, function (key, data) {
            var html_data =
                '<div class="row"><input type="hidden" id="marks_id form-control" readonly name="edit[' +
                no +
                '][marks_id]" value="' +
                data.id +
                '"/><div class="row mx-2"><input type="hidden" id="marks_id form-control" readonly name="edit[' +
                no +
                '][exam_id]" value="' +
                data.timetable.exam_id +
                '"/><div class="row mx-2"><input type="hidden" id="marks_id form-control" readonly name="edit[' +
                no +
                '][student_id]" value="' +
                row.student_id +
                ...
                no +
                '][passing_marks]" value="' +
                data.timetable.passing_marks +
                '"/><div class="form-group col-sm-12 col-md-4"><input type="text" class="subject_name form-control" readonly name="edit[' +
                no +
                '][subject_name]" value="' +
                data.subject.name +
                '" /></div><div class="form-group col-sm-12 col-md-4"><input type="text" class="total_marks form-control" readonly name="edit[' +
                no +
                '][total_marks]" value="' +
                data.timetable.total_marks +
                '" /></div><div class="form-group col-sm-12 col-md-4"><input type="text" class="obtained_marks form-control" name="edit[' +
                no +
        ...
        if (row.timetable.length != 0) {
            $.each(row.timetable, function (key, value) {
                ...
                if (!$(".edit-timetable-container:last").is(":empty")) {
                    html = $(".edit-timetable-container")
                        .find(".edit_exam_timetable:last")
                        .clone();
                } else {
                    html = $(".edit_exam_timetable_tamplate").clone();
                }
                html.addClass("edit_exam_timetable").removeClass(
                    "edit_exam_timetable_tamplate"
                );
                ...
                html.find(".remove-edit-exam-timetable-content").attr(
                    "data-timetable_id",
                    value.id
                );
            ...
            $(document).on(
                "click",
                ".remove-edit-exam-timetable-content",
                function (e) {
                    e.preventDefault();
...
window.FeesTypeActionEvents = {
    "click .edit-data": function (e, value, row, index) {
        ...
        $("#edit_description").val(row.description);
        if (row.choiceable) {
            $("#edit_choiceable_true")
                .val(row.choiceable)
                .attr("checked", true);
            $("#edit_choiceable_false").attr("checked", false);
        } else {
            $("#edit_choiceable_false")
                .val(row.choiceable)
                .attr("checked", true);
    ...
    "click .edit-data": function (e, value, row, index) {
        ...
        if (row.fees_type.length) {
            ...
            $.each(row.fees_type, function (key, value) {
                ...
                fees_type
                    .find("select")
                    .find("option[value = '" + value.fees_type_id + "']")
                    .attr("selected", "selected");
                fees_type.find(".edit_amount").val(value.amount);
                if (value.choiceable) {
                    fees_type
                        .find("#editChoiceableNo_" + (key + 1))
                        .removeAttr("checked");
                    fees_type
                        .find("#editChoiceableYes_" + (key + 1))
                        .attr("checked", true);
                } else {
                    fees_type
                        .find("#editChoiceableYes_" + (key + 1))
                ...
                fees_type
                    .find(".edit_choiceable")
                    .val(value.choiceable)
                    .attr("checked", true);
...
window.feesPaidEvents = {
    "click .compulsory-data": function (e, value, row, index) {
        ...
        if (row.compulsory_fees != null) {
            ...
            $.each(row.compulsory_fees, function (index, value) {
                html +=
                    '<tr><td scope="row" class="text-left"></td><td colspan="2" class="text-left">' +
                    value.fees_type.name +
            ...
            if (row.due_charges.charges) {
                html +=
                    '<tr class="due_charges_whole_year"><td scope="row" class="text-left"></td><td colspan="2" class="text-left">' +
                        lang_due_charges +
                        "<br><small>" +
                        lang_date +
                        " :- (" +
                        row.due_charges.date +
                        ')</small></td><td class="text-right">' +
                        row.due_charges.charges ??
                    row.due_charges.charges.toFixed(2) + "</td></tr>";
            }

            // Pay In Installment Tick
            if (row.due_charges) {
                html +=
                    '<tr class="pay_in_installment_row"><td scope="row" class="text-left"></td><td colspan="2" class="text-left">' +
                    lang_pay_in_installment +
                    '</td><td class="text-right"><input type="checkbox" name="pay_in_installment" class="form-check-input pay_in_installment" value="" data-base_amount="' +
            ...
            if (row.installment_data.length) {
                $.each(
                    row.installment_data,
                    function (index, installment_data) {
            ...
            if (row.installment_data.length) {
                ...
                let installment_amount = (
                    ...
                    Number(row.installment_data.length)
                ).toFixed(2);
                $.each(
                    row.installment_data,
                    function (index, installment_data) {
                        ...
                        if (installment_data.due_charges_applicable) {
                            let due_charges = (
                                (installment_amount *
                                    Number(installment_data.due_charges)) /
                                100
                            ).toFixed(2);
                            let total_installment_amount = (
                                Number(installment_amount) + Number(due_charges)
                            ).toFixed(2);
                            total = (
                                Number(total) + Number(total_installment_amount)
                            ).toFixed(2);
                            // if the data is last
                            if (row.installment_data.length == index + 1) {
                                ...
                                if (installment_data.paid) {
                                    ...
                                    if (paid_installment_count == index) {
                                        ...
                                        html +=
                                            '<tr class="installment_rows"><td scope="row" class="text-left"><span class="remove-installment-fees-paid text-left" data-id="' +
                                            installment_data.paid_id +
                                            '"><i class="fa fa-times text-danger" style="cursor:pointer" aria-hidden="true"></i></span></td><td colspan="2" class="text-left"><lable>' +
                                            installment_data.name +
                                            '<lable><br><small class="text-success">' +
                                            lang_paid_on +
                                            ": " +
                                            installment_data.paid_on +
                                            '</small></td><td class="text-right"><lable>' +
                                            total_installment_amount +
                                            "</lable></td></tr>";
                                    } else {
                                        html +=
                                            '<tr class="installment_rows"><td scope="row" class="text-left"></td><td colspan="2" class="text-left"><lable>' +
                                            installment_data.name +
                                            '<lable><br><small class="text-success">' +
                                            lang_paid_on +
                                            ": " +
                                    ...
                                    // show the check box
                                    html +=
                                        '<tr class="installment_rows"><td scope="row" class="text-left"><input type="checkbox" name="installment_fees[' +
                                        index +
                                        '][id]" class="installment-chkclass" value="' +
                                        installment_data.id +
                                        '" data-amount="' +
                                        total_installment_amount +
                                        '"></td><td colspan="2" class="text-left"><lable>' +
                                        installment_data.name +
                                        '<lable><br><small class="text-danger">' +
                                        lang_due_date_on +
                                        ": " +
                                        ...
                                        "</small><br><hr>" +
                                        total_installment_amount +
                                        '</lable><input type="hidden" name="installment_fees[' +
                                        index +
                                        '][amount]" value="' +
                                        total_installment_amount +
                                        '"><input type="hidden" name="installment_fees[' +
                                        index +
                                        '][fully_paid]" value="1"><input type="hidden" name="installment_fees[' +
                                        index +
                                        '][due_charges]" value="' +
                                        due_charges +
                                        '"></td></tr>';
                                ...
                                if (installment_data.paid) {
                                    ...
                                    html +=
                                        ...
                                        '"><input type="hidden" name="installment_fees[' +
                                        index +
                                        '][fully_paid]" value="0"><input type="hidden" name="installment_fees[' +
                                        index +
                                        '][due_charges]" value="' +
                                        due_charges +
                                        '"></td></tr>';
                            ...
                            // if the data is last
                            total = (
                                Number(total) + Number(installment_amount)
                            ).toFixed(2);
                            if (row.installment_data.length == index + 1) {
                                if (installment_data.paid) {
                                    if (paid_installment_count == index) {
            ...
            html +=
                '<tr><td scope="row" class="text-left"></td><td colspan="2" class="text-left"></td><td class="text-right"><hr></td></tr>';
            ...
            if (row.due_charges.charges) {
                html +=
                    '<tr><td scope="row" class="text-left"></td><td colspan="2" class="text-left">' +
                    lang_total_amount +
                    ' </td><td class="text-right compulsory_amount">' +
                    row.base_amount_with_due_charges.toFixed(2) +
                    "</td></tr>";
            } else {
                html +=
                    '<tr><td scope="row" class="text-left"></td><td colspan="2" class="text-left">' +
    ...
    "click .optional-data": function (e, value, row, index) {
        ...
        if (
            row.mode == 1 &&
            (row.type_of_fee == 2 || row.type_of_fee == null)
        ) {
            $(document).find(".mode_container").show(200);
        ...
        if (row.choiceable_fees.length) {
            ...
            $.each(row.choiceable_fees, function (index, value) {
                ...
                if (value.is_paid) {
                    if (value.date) {
                        html +=
                            '<tr><td scope="row" class="text-left"><span class="remove-optional-fees-paid text-left" data-id="' +
                            value.paid_id +
                            '"><i class="fa fa-times text-danger" style="cursor:pointer" aria-hidden="true"></i></span></td><td colspan="2" class="text-left">' +
                            value.name +
                            '<br><span class="text-small text-success">(' +
                            lang_paid_on +
                            " :- " +
                            value.date +
                    ...
                    count = count + 1;
                    html +=
                        '<tr><td scope="row" class="text-left"><input type="checkbox" class="chkclass" name="optional_fees_type_data[' +
                        index +
                        '][id]" value="' +
                        value.fees_type_id +
                        '" data-amount="' +
                        value.amount +
                        '"></td><td colspan="2" class="text-left">' +
                        value.name +
                        '</td><td class="text-right">' +
                        value.amount.toFixed(2) +
                        '<input type="hidden" name="optional_fees_type_data[' +
                        index +
                        '][amount]" value=' +
                        value.amount +
                        "></td></tr>";
            ...
            // Add Total Amount Section
            html +=
                '<tr><td></td><td colspan="2" class="text-left">' +
                lang_total_amount +
                ' </td><td class="text-right"><strong><span class="optional_total_amount_label"></span></strong><input type="hidden" name="total_amount" class="optional_total_amount"></td></tr></tbody></table>';
            ...
            if (count > 0) {
                ...
                $(".optional_total_amount_label").html(
                    parseFloat(total).toFixed(2)
                );
            }

            // Get the Amount Of Total Amount From DIV
            let choice_amount = parseInt(
                $(".optional_total_amount_label").html()
            );
    ...
    "click .edit-data": function (e, value, row, index) {
        ...
        if (
            row.fees_class_choiceable_data != null ||
            row.fees_class_paid_choiceable_data != null
        ) {
            $(".edit_choiceable_div").show();
            let html = "";
            html +=
                '<div class="form-check form-check-inline"><label class="edit_paid_amount" data-amount=' +
                row.total_fees +
                ">Paid amount - " +
                row.total_fees +
                "</label></div>";
            $.each(row.fees_class_choiceable_data, function (index, value) {
                html +=
                    '<div class="form-check form-check-inline"><label class="form-check-label"><input type="checkbox" name="add_new_choiceable_fees[]" class="form-check-input edit_new_chkclass" value="' +
                    value.fees_type_id +
                    '" data-amount="' +
                    value.amount +
                    '">' +
            ...
            if (row.fees_class_paid_choiceable_data != null) {
                html += "<hr>";
                $.each(
                    row.fees_class_paid_choiceable_data,
                    function (index, value) {
                        html +=
                            '<div><label><a href="#" data-id=' +
                            value.id +
                            " data-amount=" +
                            value.amount +
                            ' style="color:red" class="remove-paid-choiceable-fees"><i class="fa fa-remove"></i></a> ' +
                            value.fees_type.name +
                            " - " +
                            value.amount +
                            "</label></div>";
    ...
    "click .edit-data": function (e, value, row, index) {
        ...
        <div class="remove-edit-option-content">
            ...
            $.each(row.options, function (index, value) {
                if (index >= 2) {
                    html =
                        '<div class="form-group col-md-6"><input type="hidden" class="edit_option_id" name="edit_options[' +
                        (index + 1) +
                        '][id]" value=' +
                        value.id +
                        "><label>" +
                        lang_option +
                        ' <span class="edit-option-number"> ' +
                        (index + 1) +
                        '</span> <span class="text-danger">*</span></label><input type="text" name="edit_options[' +
                        (index + 1) +
                        '][option]" value="' +
                        value.option +
                        '" placeholder="' +
                        lang_enter_option +
                        '" class="form-control add-edit-question-option" /><div class="remove-edit-option-content"><button type="button" class="btn btn-inverse-danger remove-edit-option btn-sm mt-1" data-id="' +
                        value.id +
                    ...
                    html =
                        ...
                        '" placeholder="' +
                        lang_enter_option +
                        '" class="form-control add-edit-question-option" /><div class="remove-edit-option-content"></div></div>';
                }
                    ...
                    if (value.id == answer_value.option_id) {
                        if (row.answers.length == 1) {
                            ...
                            let html =
                                ...
                                " " +
                                (index + 1) +
                                ' <span class="fa fa-times text-danger remove-answers" data-id=' +
                                answer_value.id +
        ...
        if (row.options_not_answers) {
            $.each(row.options, function (index, value) {
                $.each(
                    row.options_not_answers,
                    function (answer_index, option_data) {
                        if (value.id == option_data.id) {
                            $(".edit_answer_select").append(
                                '<option value="' +
                                    option_data.id +
                                    '">' +
                                    lang_option +
...
window.classEvents = {
    "click .edit-data": function (_e, _value, row) {
        ...
        $("input[name=medium_id][value=" + row.medium_id + "].edit").prop(
            "checked",
            true
        );

        row.section_id.forEach(function (data) {
            $('input[name="section_id[]"][value=' + data + "].edit").prop(
                "checked",
                true
            );
        });
...
window.sessionYearEvents = {
    "click .editdata": function (e, value, row, index) {
        $("#id").val(row.id);
        $("#name").val(row.name);
        $("#free_app_use_date").val(
            row.free_app_use_date ? row.free_app_use_date : ""
        );
        ...
        if (row.fee_installments.length) {
            ...
            $.each(row.fee_installments, function (key, value) {
                if (!$(".edit-installment-container:last").is(":empty")) {
                    html = $(".edit-installment-container")
                        .find(".edit-installment-content:last")
                        .clone();
                } else {
                    html = $(".edit-installment-content-template").clone();
                }
                html.addClass("edit-installment-content").removeClass(
                    "edit-installment-content-template"
                );
                html.css("display", "block");
                html.find(".error").remove();
                html.find(".has-danger").removeClass("has-danger");
                ...
                html.find("#editInstallmentDueDate_" + (key + 1)).val(
                    value.due_date
                );
                html.find("#editInstllmentDueCharges_" + (key + 1)).val(
                    value.due_charges
                );

                html.find(".add-edit-fee-installment-content i")
                    .addClass("fa-times")
                    .removeClass("fa-plus");
                html.find(".add-edit-fee-installment-content")
                    .addClass(
                        "btn-inverse-danger remove-edit-fee-installment-content"
                    )
                    .removeClass(
                        "btn-inverse-success add-edit-fee-installment-content"
                    );
                html.find(".remove-edit-fee-installment-content").attr(
                    "data-id",
                    value.id
                );
...
function linkFormatter(url, name, counter) {
    return `<a href="${url}" target="_blank">${counter}. ${name}</a><br>`;
}
...
function fileFormatter(value, row) {
    ...
    $.each(row.file, function (key, data) {
        ...
        switch (Number(data.type)) {
            ...
            case 5:
                // 5 = Video Corner Link
                video_corner_link += linkFormatter(
                    data.file_url,
                    data.file_name,
            ...
            case 6:
                ...
                video_corner_download_link += linkFormatter(
                    data.file_url,
                    data.file_name,
...
function assignmentFileFormatter(value, row) {
    let html =
        "<a target='_blank' href='" + row.file + "'>" + row.name + "</a>";
    return html;
}
...
function imageFormatter(value, row) {
    if (row.image) {
        return (
            ...
            "'><img src='" +
            row.image +
            "' class='img-fluid'  alt='image' onerror='onErrorImage(event)'  /></a>"
        );
    } else {
        return "-";
    }
        ...
        $.each(row.timetable, function (key, timetable) {
            html.push(
                "<p>" +
                    ...
                    "(" +
                    timetable.subject.type +
                    ")  - " +
                    timetable.total_marks +
                    "/" +
                    timetable.passing_marks +
                    " - " +
                    timetable.start_time +
                    " - " +
                    timetable.end_time +
                    " - " +
                    moment(timetable.date, "YYYY-MM-DD").format("DD-MM-YYYY") +
                    "</p>"
            );
        });
    }
...
function examStudentNameFormatter(index, row) {
    return (
        "<input type='hidden' name='exam_marks[" +
        row.no +
        "][student_id]' class='form-control' value='" +
        row.student_id +
        "' />" +
        row.student_name
    );
...
function obtainedMarksFormatter(index, row) {
    if (row.obtained_marks) {
        return (
            "<input type='hidden' name='exam_marks[" +
            row.no +
            "][exam_marks_id]' class='form-control' value='" +
            row.exam_marks_id +
            "' />" +
            "<input type='number' name='exam_marks[" +
            row.no +
            "][obtained_marks]' class='form-control' min='0' value='" +
            row.obtained_marks +
            "' />" +
            "<input type='hidden' name='exam_marks[" +
            row.no +
            "][total_marks]' class='form-control' value='" +
            parseInt(row.total_marks) +
            "' />"
        );
    } else {
        return (
...
function teacherReviewFormatter(index, row) {
    if (row.teacher_review) {
        return (
            "<textarea name='exam_marks[" +
            row.no +
            "][teacher_review]' class='form-control'>" +
            row.teacher_review +
            "</textarea>"
        );
    } else {
    ...
    $.each(row.elective_subject_groups, function (key, group) {
        ...
        $.each(group.elective_subjects, function (key, elective_subject) {
            html +=
                ...
                ". " +
                elective_subject.subject.name +
                " - " +
                elective_subject.subject.type +
                "<br>";
            elective_subject_count++;
        });
        html += "<b>Total Subjects : </b>" + group.total_subjects + "<br>";
        html +=
            "<b>Total Selectable Subjects : </b>" +
...
function feesPaidModeFormatter(index, row) {
    if (row.mode != null) {
        if (row.mode == 0) {
            ...
            return (
                "<span class='badge badge-warning'>" + lang_cheque + "</span>"
            );
        } else {
            return (
                "<span class='badge badge-success'>" + lang_online + "</span>"
            );
        }
    }
}
...
function questionTypeFormatter(index, row) {
    if (row.question_type) {
        ...
        return (
            "<span class='badge badge-info'>" + lang_simple_question + "</span>"
        );
    }
}
...
function optionsFormatter(index, row) {
    ...
    $.each(row.options, function (index, value) {
        html += "<div class='row'>";
        html +=
            "<div class= 'col-md-1 text-center'><i class='fa fa-arrow-right small' aria-hidden='true'></i></div><div class='col-md-6'>" +
            value.option +
            "</div><br>";
        html += "</div>";
    });
...
function answersFormatter(index, row) {
    ...
    $.each(row.answers, function (index, value) {
        html += "<div class='row'>";
        html +=
            "<span class= 'col-md-1 text-center'><i class='fa fa-arrow-right small' aria-hidden='true'></i></span><div class='col-md-6'>" +
            value.answer +
            "</div><br>";
        html += "</div>";
    });
...
function feesPaidStatusFormatter(value, row, index) {
    if (row.fees_status == 1) {
        ...
        return (
            "<span class='badge badge-info'>" + lang_partial_paid + "</span>"
        );
    } else {
        return "<span class='badge badge-warning'>" + lang_pending + "</span>";
    }
...
window.leavesEvents = {
    "click .edit-data": function (e, value, row) {
        ...
        $("input[name=status][value=" + row.status + "].leave-status").prop(
            "checked",
            true
        );
    },
