        ...
        <a class="lesson-topics" href="{{ route('lesson.topics', $lesson->id) }}">
            <div class="card">
                <img style="width: 364px; height: 263px; margin-left:auto; margin-right:auto;" class="card-img-top"
                    src="{{ !empty($lesson->thumbnail) ? $lesson->thumbnail : loadTenantMainAsset('logo1', global_asset('images/no_image_available.jpg')) }}"
                    alt="{{ $lesson->name }}">
                <div class="card-body text-center">
                    <h4 class="card-title">{{ $lesson->name }}</h4>
                    <p>{{ str()->limit($lesson->description, 90) }}</p>
                </div>
                    ...
                    @if (auth()->check() && auth()->user()->student)
                        @if (empty($lesson->studentActiveEnrollment))
                            @if ($lesson->is_lesson_free)
                                <button data-class-section-id ="{{ $lesson->class_section_id }}"
                                    data-id="{{ $lesson->id }}" class="btn btn-success locked-btn mt-3"<i
                                    class="fa fa-gift mx-2"></i>Enroll Lesson For Free!</button>
                            @else
                                <button data-class-section-id ="{{ $lesson->class_section_id }}"
                                    data-id="{{ $lesson->id }}" class="btn btn-success locked-btn mt-3"
                                    data-price="{{ $lesson->price }}" data-bs-toggle="modal"
                                    data-bs-target="#payment-methods"><i class="fa fa-shopping-cart mx-2"></i>Enroll
                                    Lesson!</button>
                            @endif
                        @endif
                    @elseif(!auth()->check())
